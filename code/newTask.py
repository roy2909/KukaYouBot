...
def log(message):
    with open('logfile_newTask.txt', 'a') as file:
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        file.write(f"{timestamp}: {message}\n")

def newTask():
    log("Starting the 'newTask' function")
    ...
    for i in range(0, len(traj)-1):
        ...
        Xerr_plot[i] = Xerr # Store the error

    # Save configuration and error data to files
    log("Generating newTask.csv file")
    np.savetxt("newTask.csv", config, delimiter = ",")
    log("Generating X_error_newTask.csv file")
    np.savetxt("X_error_newTask.csv", Xerr_plot, delimiter = ",")
