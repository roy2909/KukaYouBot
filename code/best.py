...
def log(message):
    with open('logfile_best.txt', 'a') as file:
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        file.write(f"{timestamp}: {message}\n")

def best():
    log("Starting the 'best' function")
    ...
    #The best proportional and integral gains
    Kp = np.identity(6) * 3.0 
    ...
    for i in range(0, len(traj)-1):
        ...
        Xerr_plot[i] = Xerr
    log("Generating best.csv file")
    np.savetxt("best.csv", config, delimiter = ",")
    log("Generating X_error_BEST.csv file")
    np.savetxt("X_error_BEST.csv", Xerr_plot, delimiter = ",")
    ...
    plt.title(f'X error with time with Kp= 3.0 and Ki= 0.01')
